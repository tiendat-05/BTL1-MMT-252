...
class Response():   
    ...
    def build_response_header(self, request):
        ...
        headers = {
            ...
            "Content-Type": "{}".format(self.headers.get('Content-Type', 'application/json')),
        ...
        # Kế thừa Authorization từ request nếu có
        if "Authorization" in reqhdr:
            headers["Authorization"] = reqhdr["Authorization"]
        else:
            headers["Authorization"] = "Basic <credentials>"

        # Header text alignment
        # Bước 1: Khởi tạo danh sách các dòng header với Status Line đầu tiên
        status_code = getattr(self, 'status_code', 200) or 200
        reason = getattr(self, 'reason', 'OK') or 'OK'
        header_lines = [f"HTTP/1.1 {status_code} {reason}"]

        # Bước 2: Thêm các header từ dictionary vào danh sách
        for key, value in headers.items():
            header_lines.append(f"{key}: {value}")

        # Bước 3: Thêm các Header Set-Cookie (để xử lý session từ Task 2)
        if hasattr(self, 'cookies') and self.cookies:
            for k, v in self.cookies.items():
                header_lines.append(f"Set-Cookie: {k}={v}; Path=/")

        # Bước 4: Nối các dòng bằng ký tự CRLF và kết thúc khối header bằng 2 lần CRLF
        fmt_header = "\r\n".join(header_lines) + "\r\n\r\n"
        
        # Cập nhật _header và trả về dạng bytes
